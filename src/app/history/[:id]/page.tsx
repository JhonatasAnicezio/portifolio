...
export default async function Posts({ params }: PostPageProps) {
    // GET PROJECT
    // -- //
    const docRef = doc(db, 'history', params[':id']);
    ...
    // -- //
    const historyIds = (await getDocs(collectionRef))
        .docs.map(doc => doc.id);
        
    const { previousId, nextId } = historyIds.reduce((acc: {
    ...
    if (!project || !historyIds) {
        redirect('/history');
    }

    return (
        <div className="flex h-screen w-full items-center justify-center fixed left-0 top-0">
            <Link className="absolute h-screen w-full z-10 bg-black opacity-50 cursor-default" href={'/history'} />
